const getLane = (laneIndex) => {
    const visibleIndex = laneIndex + 1;
    return `
        <div class="lane-wrapper">
            <div>Lane #${ visibleIndex }</div>
            <div class="lane-container">
                <svg id="lane-${ laneIndex }" class="lane" name="Lane ${ visibleIndex }" height="100" width="600">
                    <svg id="car-${ laneIndex }" name="Car ${ visibleIndex }" preserveAspectRatio="xMinYMin meet" viewBox="0 0 511 511">
                        <path d="M160,307.5h133.538c14.182,0,27.524-7.554,34.82-19.715l14.501-24.169c2.273-3.789,1.045-8.703-2.744-10.976 c-3.788-2.273-8.702-1.045-10.976,2.744l-14.501,24.169c-4.421,7.369-12.507,11.947-21.101,11.947H160c-4.418,0-8,3.582-8,8 S155.582,307.5,160,307.5z"/>
                        ...
                        <path d="M112,291.5c-4.418,0-8,3.582-8,8c0,8.822-7.178,16-16,16s-16-7.178-16-16s7.178-16,16-16c4.418,0,8-3.582,8-8s-3.582-8-8-8 c-17.645,0-32,14.355-32,32s14.355,32,32,32s32-14.355,32-32C120,295.082,116.418,291.5,112,291.5z"/>
                        <path d="M440,291.5c-4.418,0-8,3.582-8,8c0,8.822-7.178,16-16,16s-16-7.178-16-16s7.178-16,16-16c4.418,0,8-3.582,8-8s-3.582-8-8-8 c-17.645,0-32,14.355-32,32s14.355,32,32,32s32-14.355,32-32C448,295.082,444.418,291.5,440,291.5z"/>
                    </svg>
                </svg>
...
const createSelectOption = (optionType, index) => {
    const option = document.createElement('option');
    const elementId = optionType + "-" + index;
    const displayText = toTitleCase(`${optionType} ${index + 1}`);
    option.setAttribute("value", elementId);
    option.innerHTML = displayText;
    return option;
};

const addToSelection = ( index ) => {
    const selection = document.querySelector("#active-element");
    const carOption = createSelectOption("car", index);
    const laneOption = createSelectOption("lane", index);
    selection.appendChild( carOption );
    selection.appendChild( laneOption );
};
...
const removeFromSelection = ( index ) => {
    ...
    options.forEach( option => {
        if( option.value.includes( index ) ){
...
const addLane = ( index ) => {
    const newLaneMarkup = getLane(index);
    const newLaneElement = htmlToElement(newLaneMarkup);
    const track = document.getElementById("track");
    track.appendChild( newLaneElement );
    addToSelection( index );
};

const removeLane = ( lane, index ) => {
    const track = document.getElementById("track");
    track.removeChild( lane );
    removeFromSelection( index );
};

const adjustLanes = ( newLaneCount ) => {
    if( newLaneCount > 0 ){
        const currentLanes = document.querySelectorAll(".lane-wrapper");
        const currentLanesCount = currentLanes.length;
      
      if( currentLanesCount < newLaneCount ){
        for(let i = currentLanesCount; i < newLaneCount; i++){
            addLane( i );
        }
      } else if( currentLanesCount > newLaneCount ) {
        currentLanes.forEach((lane, index) => {
            if( index > newLaneCount - 1 ) removeLane( lane, index );
        })
      }
    }
